    ...
    for line in input.lines() {
        for p in line.split(' ') {
            match p {
                ...
                "turn"      => builder.wait(),
                "on"        => builder.turn_on(),
                "off"       => builder.turn_off(),
                "through"   => builder.through(),
                _           => builder.input(&p)
            }
        }
        builder.execute::<BinaryCommand>(&mut grid1);
        builder.execute::<DiscreteCommand>(&mut grid2);
    }
    let mut lights_on = 0;
    let mut brightess = 0;
    for light in 0..grid1.len() { lights_on += grid1[light] }
    for light in 0..grid2.len() { brightess += grid2[light] }
    (lights_on, brightess)
}

struct GridRange { x0: usize, y0: usize, x1: usize, y1: usize }

impl GridRange {
    fn new() -> Self {
        GridRange { x0: 0, y0: 0, x1: 0, y1: 0 }
    }

    fn area(&self) -> Vec<usize> {
        (self.y0 .. self.y1 + 1).flat_map(|y| Self::row(self.x0, self.x1, y)).collect()
    }

    fn locate(x: usize, y: usize) -> usize {
        y * 1000 + x
    }

    fn row(x0: usize, x1: usize, y: usize) -> ::std::ops::Range<usize> {
        (Self::locate(x0, y) .. Self::locate(x1, y) + 1)
    }
}

enum State { Wait, FirstInput, SecondInput }
...
impl InstructionBuilder {
    fn new() -> Self {
        InstructionBuilder { state: State::Wait, command: "none", range: GridRange::new() }
    }
    ...
    fn toggle(&mut self) {
        self.state   = State::FirstInput;
        self.command = "toggle";
    }

    fn turn_on(&mut self) {
        self.state   = State::FirstInput;
        self.command = "turn on";
    }

    fn turn_off(&mut self) {
        self.state   = State::FirstInput;
        self.command = "turn off";
    }

    fn through(&mut self) {
        self.state = State::SecondInput;
    }

    fn input(&mut self, expr: &str) {
        match self.state {
            State::Wait         => panic!("Invalid sequence"),
            State::FirstInput   => { let coords = Self::parse_coords(&expr); self.range.x0 = coords.0; self.range.y0 = coords.1; },
            State::SecondInput  => { let coords = Self::parse_coords(&expr); self.range.x1 = coords.0; self.range.y1 = coords.1; }
        }
        self.state = State::Wait;
    }

    fn execute<T: Command>(&self, grid: &mut Grid) {
        T::execute(self.command, &self.range, grid);
    }

    fn parse_coords(expr: &str) -> (usize, usize) {
        let mut p = expr.split(',').map(|v| v.parse::<usize>().unwrap());
        (p.nth(0).unwrap(), p.nth(0).unwrap())
    }
}

type Grid = [i32;1000000];

trait Command {
    fn execute(name: &str, range: &GridRange, grid: &mut Grid);
}
...
impl Command for BinaryCommand {
    fn execute(name: &str, range: &GridRange, grid: &mut Grid) {
        match name {
            "toggle"   => for i in range.area() { if grid[i] == 0 { grid[i] = 1 } else { grid[i] = 0 } },
            "turn on"  => for i in range.area() { grid[i] = 1 },
            "turn off" => for i in range.area() { grid[i] = 0 },
            _          => panic!(format!("Unknown command: {}", name))
        }
    }
}
...
impl Command for DiscreteCommand {
    fn execute(name: &str, range: &GridRange, grid: &mut Grid) {
        match name {
            "toggle"   => for i in range.area() { grid[i] += 2 },
            "turn on"  => for i in range.area() { grid[i] += 1 },
            "turn off" => for i in range.area() { if grid[i] > 0 { grid[i] -= 1 } },
            _          => panic!(format!("Unknown command: {}", name))
        }
    }
}
